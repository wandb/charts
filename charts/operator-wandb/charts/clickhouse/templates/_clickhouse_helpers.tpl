...
*/}}
{{- define "clickhouse.containerPorts" -}}
{{- range .Values.container.clickhouse.ports }}
- name: {{ .name }}
...
{{- define "clickhouse.raftServer" -}}
{{- range $i, $e := until (int .Values.replicaCount) }}
<server>
    <id>{{ $i }}</id>
    <hostname>ch-keeper-{{ $i }}.{{ include "clickhouse.fullname" $ }}-headless.svc.cluster.local</hostname>
    <port>{{ include "clickhouse.raftPort" $ }}</port>
</server>
{{- end }}
{{- end }}
